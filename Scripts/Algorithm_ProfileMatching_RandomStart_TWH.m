function [] = Algorithm_ProfileMatching_RandomStart_TWH(tile_name,lake_id,data_dir,output_dir,base_lake_id)
% This algorithm uses TWH criteria to break the ties between locations assigned to same elevation

base_name = ['PMR_TWH_'  tile_name '_' lake_id]; % profile matching with Greedy start and TWH as the criteria for breaking ties
load([data_dir '/Stack_' tile_name '_' base_lake_id '.mat']);
[N,T] = size(mapStack);
